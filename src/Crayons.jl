...
abstract AbstractCrayon

function Base.print(io::IO, x::AbstractCrayon)
...
function Base.show(io::IO, x::AbstractCrayon)
    if anyactive(x) 
        print(io, x)
...
anyactive(::AbstractCrayon) = true

immutable ResetCrayon <: AbstractCrayon
end
_print(io::IO, ::ResetCrayon) = print(io, '0')


immutable Crayon <: AbstractCrayon
    fg::ANSIColor
    bg::ANSIColor
